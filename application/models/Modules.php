...
class Modules extends CI_Model {
    ...
    function new_summary_page($data) {
        return $this->db->insert('html_content_summary_page_tbl', $data);
    }
    ...
    function test_trial_data($id, $date, $client, $key, $secret, $timestamp) {
        ...
        foreach ($Row as $key => $value) {
            if ($value['RowType'] == "Header") {
                ...
                if ($value['Title'] == "Revenue") {
                    $getdata = $value['Rows']['Row'];

                    foreach ($getdata as $key => $getcell) {
                        $count = 0;
                        //print_r($getcell);
                        if (isset($getcell['Cells']['Cell'])) {

                            $cellvalue = $getcell['Cells']['Cell'];
                            foreach ($cellvalue as $key => $cellextract) {

                                if (isset($cellextract['Value'])) {
                                    $cell = $cellextract['Value'];
                                } else {
                                    $cell = 0;
                                }
                                ...
                                if ($count == 5) {
                                    ...
                                    $sql = "INSERT INTO trial_balance_logs ()VALUES "
                                            . "('$get_account', '$title',  '$debit_account', '$credit_account', '$debit_ytd', '$credit_ytd')";
                                }
                            }
                        }
                        if (isset($getcell['Cell'])) {
                    ...
                    foreach ($getRow as $key => $valuex) {
                        if (isset($valuex['RowType'])) {
                            if ($valuex['RowType'] == "Row") {

                                $getCells = $valuex['Cells']['Cell'];

                                $count = 0;
                                foreach ($getCells as $key => $valuecell) {
                                    $count++;

                                    if (isset($valuecell['Value'])) {
    ...
    function get_trial_balance($id, $date, $client, $key, $secret, $loop, $clientname) {
        ...
        $reference = $client . "-" . $timestamp;
        //	$reference = "51-11132018040043";

        $date_create_value = "";
        $date_crete_value_two = "";
        $date_printxx = "";
        ...
        for ($x = 0; $x <= $loop; $x++) {
            ...
            $new_var_Date = explode("-", $newdate);

            $date_print = $new_var_Date[2] . '-' . $new_var_Date[0] . "-" . $new_var_Date[1];
            $lastday = date('t', strtotime($date_print));
            $date_print_new = $new_var_Date[0] . '/' . $lastday . "/" . $new_var_Date[2];
            $date_print_insert = $new_var_Date[0] . '-' . $lastday . "-" . $new_var_Date[2];


            $date_printxx = $date_printxx . $date_print_new . "|";
            $fit_date_to_function = $new_var_Date[0] . '-' . $lastday . "-" . $new_var_Date[2];
            $this->test_trial_data($id, $date_print_insert, $client, $key, $secret, $timestamp);
        }


        $date_splice = explode("|", $date_printxx);

        $sql_query = "SELECT account_type FROM trial_balance_logs
				where   reference='" . $reference . "'  GROUP BY account_type";
        ...
        <table cellSpacing="0" cellpadding="0" width="100%">
            ...
            <tr>
                <td align="center"><span style="font-weight: bold;" >Income Statements</span></td>
            </tr>
            <tr>
                <td align="center"><span style="font-weight: bold;" >For the YTD ended as of <?php echo $this->date_caption_convert_full($date_required[0] . "/" . $date_required[1] . "/" . $date_required[2]); ?></span></td>
            </tr>
            <tr>
                <td align="center">&nbsp;</td>
            </tr>
        ...
        <table cellpadding="0" cellspacing="0" width="100%">
            <tr>
                <td class="">&nbsp;</td>
                <td style="text-align:right;font-weight: bold;"><?php echo $this->date_caption_convert($date_splice[5]); ?></td>
                <td style="text-align:right;font-weight: bold;"><?php echo $this->date_caption_convert($date_splice[4]); ?></td>
                <td style="text-align:right;font-weight: bold;"><?php echo $this->date_caption_convert($date_splice[3]); ?></td>
                <td style="text-align:right;font-weight: bold;"><?php echo $this->date_caption_convert($date_splice[2]); ?></td>
                <td style="text-align:right;font-weight: bold;"><?php echo $this->date_caption_convert($date_splice[1]); ?></td>
                <td style="text-align:right;font-weight: bold;"><?php echo $this->date_caption_convert($date_splice[0]); ?></td>
                <td style="text-align:right;font-weight: bold;">YTD</td>
            </tr>
            <tr>
            ...
            <?php
            //foreach ($data as $key) {
            $sql = "SELECT a.account, a.account_type,

if(isnull(b.monthly),0,b.monthly) as monthly1, if(isnull(b.yearly),0,b.yearly) as yearly1,
if(isnull(c.monthly),0,c.monthly) as monthly2, if(isnull(c.yearly),0,c.yearly) as yearly2,
if(isnull(d.monthly),0,d.monthly) as monthly3, if(isnull(d.yearly),0,d.yearly) as yearly3,
if(isnull(e.monthly),0,e.monthly) as monthly4, if(isnull(e.yearly),0,e.yearly) as yearly4,
if(isnull(f.monthly),0,f.monthly) as monthly5, if(isnull(f.yearly),0,f.yearly) as yearly5,
if(isnull(g.monthly),0,g.monthly) as monthly6, if(isnull(g.yearly),0,g.yearly) as yearly6


FROM trial_balance_logs as a

LEFT JOIN(SELECT account, account_type,
debit  + credit  as monthly,
ytddebit  +  ytdcredit as yearly,
add_by,date
FROM trial_balance_logs
where date='" . $date_splice[5] . "'
and reference='" . $reference . "'
GROUP BY account,account_type, reference
ORDER BY account_type) as b
...
add_by,date
FROM trial_balance_logs
where date='" . $date_splice[4] . "'
and reference='" . $reference . "'
GROUP BY account,account_type, reference
ORDER BY account_type) as c
...
add_by,date
FROM trial_balance_logs
where date='" . $date_splice[3] . "'
and reference='" . $reference . "'
GROUP BY account,account_type, reference
ORDER BY account_type) as d
...
add_by,date
FROM trial_balance_logs
where date='" . $date_splice[2] . "'
and reference='" . $reference . "'
GROUP BY account,account_type, reference
ORDER BY account_type) as e
...
add_by,date
FROM trial_balance_logs
where date='" . $date_splice[1] . "'
and reference='" . $reference . "'
GROUP BY account,account_type, reference
ORDER BY account_type) as f
...
add_by,date
FROM trial_balance_logs
where date='" . $date_splice[0] . "'
and reference='" . $reference . "'
GROUP BY account,account_type, reference
ORDER BY account_type) as g
ON a.account = g.account


where reference='" . $reference . "'
GROUP BY a.account,a.account_type, a.reference
ORDER BY a.account_type";
            ...
            foreach ($datax as $keyx) {
                if ($keyx->account == "Other Ordinary Income (471)" || $keyx->account == "Sales (400)" || $keyx->account == "Sales (DEAR) (1)" || $keyx->account == "Sales of Product Income - DEAR (408)" || $keyx->account == "Sales of Product Income - DEAR (DEAR) (58)"
                ) {
                    $monthly6 = $monthly6 + $keyx->monthly6;
                    $monthly5 = $monthly5 + $keyx->monthly5;
                    $monthly4 = $monthly4 + $keyx->monthly4;
                    $monthly3 = $monthly3 + $keyx->monthly3;
                    $monthly2 = $monthly2 + $keyx->monthly2;
                    $monthly1 = $monthly1 + $keyx->monthly1;
                    $yearly6 = $yearly6 + $keyx->yearly5;
                    ?>
            ...
            <tr>
                <td>&nbsp;Sales</td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthly1))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthly2))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthly3))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthly4))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthly5))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthly6))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($yearly6))); ?></td>

            </tr>
            <tr>
                <td style="font-weight: bold;">Total Revenue</td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round(abs($monthly1))); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round(abs($monthly2))); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round(abs($monthly3))); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round(abs($monthly4))); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round(abs($monthly5))); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round(abs($monthly6))); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round(abs($yearly6))); ?></td>
            ...
            $yearlycogs6 = 0;
            foreach ($datax as $keyx) {
                if ($keyx->account == "Cost of Goods Sold - Variance (513)" || $keyx->account == "Cost of Goods Sold (500)" || $keyx->account == "Cost of Goods Sold - DEAR (501)" || $keyx->account == "Inventory Shrinkage - DEAR (DEAR) (84)" || $keyx->account == "Dues and Subscriptions (676)" || $keyx->account == "Freight In (606)" || $keyx->account == "Supplies & Materials - COGS (512)" || $keyx->account == "Supplies and Materials - COGS (DEAR) (32)" || $keyx->account == "Purchase Discounts (514)"
                ) {
                    $monthlycogs6 = $monthlycogs6 + $keyx->monthly6;
                    $monthlycogs5 = $monthlycogs5 + $keyx->monthly5;
                    $monthlycogs4 = $monthlycogs4 + $keyx->monthly4;
                    $monthlycogs3 = $monthlycogs3 + $keyx->monthly3;
                    $monthlycogs2 = $monthlycogs2 + $keyx->monthly2;
                    $monthlycogs1 = $monthlycogs1 + $keyx->monthly1;
                    $yearlycogs6 = $yearlycogs6 + $keyx->yearly6;
                }
            }
            ?>
            <tr>
            ...
            <tr>
                ...
                <td style="text-align:right;"><?php echo number_format(round(abs($monthlycogs1))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthlycogs2))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthlycogs3))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthlycogs4))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthlycogs5))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthlycogs6))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($yearlycogs6))); ?></td>
            ...
            $yearlyp6 = 0;
            foreach ($datax as $keyx) {
                if ($keyx->account == "Production (DEAR) (98)") {
                    $monthlyp6 = $monthlyp6 + $keyx->monthly6;
                    $monthlyp5 = $monthlyp5 + $keyx->monthly5;
                    $monthlyp4 = $monthlyp4 + $keyx->monthly4;
                    $monthlyp3 = $monthlyp3 + $keyx->monthly3;
                    $monthlyp2 = $monthlyp2 + $keyx->monthly2;
                    $monthlyp1 = $monthlyp1 + $keyx->monthly1;
                    $yearlyp6 = $yearlyp6 + $keyx->yearly5;
                }
            }
            ?>
            <tr>
                <td>&nbsp;Production</td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthlyp1))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthlyp2))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthlyp3))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthlyp4))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthlyp5))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($monthlyp6))); ?></td>
                <td style="text-align:right;"><?php echo number_format(round(abs($yearlyp6))); ?></td>

            </tr>
            <tr>
                <td style="font-weight: bold;">Total Cost of Goods Sold</td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round(abs($monthlycogs1 + $monthlyp1))); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round(abs($monthlycogs2 + $monthlyp2))); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round(abs($monthlycogs3 + $monthlyp3))); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round(abs($monthlycogs4 + $monthlyp4))); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round(abs($monthlycogs5 + $monthlyp5))); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round(abs($monthlycogs6 + $monthlyp6))); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round(abs($yearlycogs6 + $monthlyp6))); ?></td>
            ...
            <tr>
                ...
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format($monthly1 - $monthlycogs1); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format($monthly2 - $monthlycogs2); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format($monthly3 - $monthlycogs3); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format($monthly4 - $monthlycogs4); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format($monthly5 - $monthlycogs5); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format($monthly6 - $monthlycogs6); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format($yearly6 - $yearlycogs6); ?></td>

            </tr>
            <tr>
                <td style="font-weight: bold;">Gross Profit %</td>
                <td style="font-weight: bold;text-align:right;"><?php echo round(($monthlycogs1 + $monthlyp1) / $monthly1 * 100); ?>%</td>
                <td style="font-weight: bold;text-align:right;"><?php echo round(($monthlycogs2 + $monthlyp2) / $monthly2 * 100); ?>%</td>
                <td style="font-weight: bold;text-align:right;"><?php echo round(($monthlycogs3 + $monthlyp3) / $monthly3 * 100); ?>%</td>
                <td style="font-weight: bold;text-align:right;"><?php echo round(($monthlycogs4 + $monthlyp4) / $monthly4 * 100); ?>%</td>
                <td style="font-weight: bold;text-align:right;"><?php echo round(($monthlycogs5 + $monthlyp5) / $monthly5 * 100); ?>%</td>
                <td style="font-weight: bold;text-align:right;"><?php echo round(($monthlycogs6 + $monthlyp6) / $monthly6 * 100); ?>%</td>
                <td style="font-weight: bold;text-align:right;"><?php echo round(($yearlycogs6 + $monthlyp6) / $yearly6 * 100); ?>%</td>
            ...
            <tr>
                <td style="font-weight: bold;">Sales and Marketing Expenses</td>
            </tr>
            <?php
            $monthlyexpense1 = 0;
                $monthlyexpense2 = 0;
            ...
            foreach ($datax as $keyx) {
                ...
                if ($keyx->account == "Product Development (DEAR) (97)" 
                        || $keyx->account == "Marketing (610)" || $keyx->account == "Commissions (602)" || $keyx->account == "Freight Out (504)" || $keyx->account == "PayPal Fees (615)" || $keyx->account == "Promotional (617)" || $keyx->account == "Shopify fees (618)" || $keyx->account == "Show Fees (619)" || $keyx->account == "Travel Meals (685)" || $keyx->account == "Website (683)"
                ) {
                    $monthlyexpense6 = $monthlyexpense6 + $keyx->monthly6;
                    $monthlyexpense5 = $monthlyexpense5 + $keyx->monthly5;
                    $monthlyexpense4 = $monthlyexpense4 + $keyx->monthly4;
                    $monthlyexpense3 = $monthlyexpense3 + $keyx->monthly3;
                    $monthlyexpense2 = $monthlyexpense2 + $keyx->monthly2;
                    $monthlyexpense1 = $monthlyexpense1 + $keyx->monthly1;
                    $yearlyexpense6 = $yearlyexpense6 + $keyx->yearly6;
                    ?>
                    <tr>
                        <td >&nbsp;<?php echo $keyx->account; ?></td>
                        <td style="text-align:right;"><?php echo number_format(round(abs($keyx->monthly1))); ?></td>
                        <td style="text-align:right;"><?php echo number_format(round(abs($keyx->monthly2))); ?></td>
                        <td style="text-align:right;"><?php echo number_format(round(abs($keyx->monthly3))); ?></td>
                        <td style="text-align:right;"><?php echo number_format(round(abs($keyx->monthly4))); ?></td>
                        <td style="text-align:right;"><?php echo number_format(round(abs($keyx->monthly5))); ?></td>
                        <td style="text-align:right;"><?php echo number_format(round(abs($keyx->monthly6))); ?></td>
                        <td style="text-align:right;"><?php echo number_format(round(abs($keyx->yearly6))); ?></td>
                    </tr>
                <?php
            }
        }
        ?>
            <tr>
                <td style="font-weight: bold;">Total Sales and Marketing Expenses</td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlyexpense1)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlyexpense2)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlyexpense3)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlyexpense4)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlyexpense5)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlyexpense6)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($yearlyexpense6)); ?></td>
            ...
            foreach ($datax as $keyx) {
                ...
                if ($keyx->account == "Bank Charges (626)" 
                    || $keyx->account == "Charitable Donation (601)"
                    || $keyx->account == "Computer and Internet (627)"
                    || $keyx->account == "Depreciation (700)"
                    || $keyx->account == "Dues and Subscriptions (676)"
                    || $keyx->account == "Freight Out (504)"
                    || $keyx->account == "Gifts (607)"
                    || $keyx->account == "Entertainment (611)"
                    || $keyx->account == "Miscellaneous (701)"
                    || $keyx->account == "Office Supplies (631)"
                    || $keyx->account == "Shopify fees (618)"
                    || $keyx->account == "Shopify fees (618)"
                    || $keyx->account == "Rent Expense (687)"
                    || $keyx->account == "Maintenance (664)"
                    || $keyx->account == "Maintenance (664)"
                    || $keyx->account == "Software Subscription (621)"
                    || $keyx->account == "Stationery and Printing (656)"
                    || $keyx->account == "Suspense (721)"
                    || $keyx->account == "Taxes (674)"
                    || $keyx->account == "Travel (684)"
                    || $keyx->account == "Travel (684)"
                    || $keyx->account == "Utilities (644)"
                    || $keyx->account == "Wages (686)"
                    || $keyx->account == "Accounting (625)"
                    || $keyx->account == "Consulting (629)"
                    || $keyx->account == "Professional Fees (630)"
                    || $keyx->account == "Subcontractors (623)"
                     ) {
                    $monthlygeneral6 = $monthlygeneral6 + $keyx->monthly6;
                    $monthlygeneral5 = $monthlygeneral5 + $keyx->monthly5;
                    $monthlygeneral4 = $monthlygeneral4 + $keyx->monthly4;
                    $monthlygeneral3 = $monthlygeneral3 + $keyx->monthly3;
                    $monthlygeneral2 = $monthlygeneral2 + $keyx->monthly2;
                    $monthlygeneral1 = $monthlygeneral1 + $keyx->monthly1;
                    $yearlygeneral6 = $yearlygeneral6 + $keyx->yearly6;
                    ?>
                    <tr>
                        <td >&nbsp;<?php echo $keyx->account; ?></td>
                        <td style="text-align:right;"><?php echo number_format(round(abs($keyx->monthly1))); ?></td>
        ...
        ?>
                     <tr>
                <td style="font-weight: bold;">Total General and Administrative Expenses</td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlygeneral1)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlygeneral2)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlygeneral3)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlygeneral4)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlygeneral5)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlygeneral6)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($yearlygeneral6)); ?></td>
            ...
            <tr>
             ...
             <tr>
                ...
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlygeneral1 + $monthlyexpense1)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlygeneral2 + $monthlyexpense2)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlygeneral3 + $monthlyexpense3)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlygeneral4 + $monthlyexpense4)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlygeneral5 + $monthlyexpense5)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($monthlygeneral6 + $monthlyexpense6)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo number_format(round($yearlygeneral6 + $yearlyexpense6)); ?></td>
            ...
            <tr>
             ...
             <tr>
                ...
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo round(($monthly1 - $monthlycogs1) - ($monthlygeneral1 + $monthlyexpense1)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo round(($monthly2 - $monthlycogs2) - ($monthlygeneral2 + $monthlyexpense2)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo round(($monthly3 - $monthlycogs3) - ($monthlygeneral3 + $monthlyexpense3)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo round(($monthly4 - $monthlycogs4) - ($monthlygeneral4 + $monthlyexpense4)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo round(($monthly5 - $monthlycogs5) - ($monthlygeneral5 + $monthlyexpense5)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo round(($monthly6 - $monthlycogs6) - ($monthlygeneral6 + $monthlyexpense6)); ?></td>
                <td style="font-weight: bold;text-align:right;border-top:1px solid black;border-bottom:1px solid black;"><?php echo round(($yearly6 - $yearlycogs6) -($yearlygeneral6 + $yearlyexpense6)); ?></td>
...
SELECT * FROM layout_pages_tbl as a LEFT JOIN(SELECT * FROM page_content_details_tbl) as b ON a.ID = b.layout_page_id where page_template='" . $id . "' and status='Active' ORDER BY sort");
    ...
    function summary_page($client) {
        return $this->db->query("SELECT * FROM html_content_summary_page_tbl where client_id='" . $client . "' ORDER BY ID DESC LIMIT 1");
    }
